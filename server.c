   ...
   pthread_mutex_lock(&lock);
    ...
    while((read_size = recv(sock , buffer , MAXDATASIZE , 0)) > 0) {
        printf("Received message: %s\n", buffer);
         sensor = atoi(buffer[0]);
         axis = atoi(buffer[1]);
         if(sensor == 1){
            switch(axis){
               case 1: 
                  sprintf(buffer, "%d", accele_data[0]);
                  break;
               case 2:
                  sprintf(buffer, "%d", accele_data[1]);
                  break;
               case 3:
                  sprintf(buffer, "%d", accele_data[2]);
                  break;
            }
         }else if(sensor == 2){
            switch(axis){
               case 1: 
                  sprintf(buffer, "%d", magne_data[0]);
                  break;
               case 2:
                  sprintf(buffer, "%d", magne_data[1]);
                  break;
               case 3:
                  sprintf(buffer, "%d", magne_data[2]);
                  break;
            }
            ...
            strcpy(buffer, "The sensor is not supported");
         }
 ...
 /* puts("listen");*/
   ...
   while(1) {
      ...
      for(i = 0; i < n_acceletomers; i++){
         ...
         accele_data[i] = acceletomer;
         magne_data[i] = magnetometer;
         gyro_data[i] = gyroscope;
      }
